  ...
  while (base::Optional<V4L2WritableBufferRef> buffer =
    ...
    switch (mem_type) {
      ...
      case V4L2_MEMORY_DMABUF: {
        ...
        if (!video_frame)
          return;
        ret = std::move(*buffer).QueueDMABuf(video_frame);
        break;
      }
      default:
        NOTREACHED();
...
void V4L2StatefulVideoDecoderBackend::OnOutputBufferDequeued(
  ...
  if (buffer->GetPlaneBytesUsed(0) > 0) {
    ...
    client_->OutputFrame(frame, *visible_rect_, timestamp);
  }
...
// TODO move into helper function shared between both backends?
